...
namespace ec2s
{
    ...
    constexpr T getEntityIndex(const Entity entity)
    {
        return static_cast<T>(entity & kEntityIndexMask);
    }

    template <typename T = Entity>
    constexpr T getEntitySlot(const Entity entity)
    {
        return static_cast<T>((entity & kEntitySlotMask) >> kEntitySlotShiftWidth);
    }

    constexpr Entity spinEntitySlot(Entity entity)
    {
        return (entity & kEntityIndexMask) | ((getEntitySlot<>(entity) + 1) << kEntitySlotShiftWidth);
    }
